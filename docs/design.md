...
## Copy is covered separately

Stage lines, button labels and the README follow [writing.md](writing.md). The game's dry, deadpan voice is deliberate, and it is the one place the stage lines are allowed to be sarcastic (see the stage-variants decision below).
...
**The call:** four selectable themes: Minimal (the shipped default), Maximalist, Terminal and Paper, swapped via a `data-theme` attribute driving CSS custom properties, persisted to `localStorage`. Not a settings panel: four small text buttons, fixed top-right, styled like everything else in the game (no icons, no modal).
